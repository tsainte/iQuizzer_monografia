  ...
  \chapter{Implementação web} \label{CHP:MET}%%
    ...
    Para a configuração do ambiente Ruby on Rails, foi utilizado um site chamado RailsInstaller \url{www.railsinstaller.org}, que implementou facilitadores para instalação em ambientes Windows e OS X. Porém, os aplicativos necessários podem ser instalados separadamente. Os necessários para a nossa aplicação foram:
\begin{itemize}
...
\item Bundler 1.2.1 - Controle de dependências (GEM's)
\end{itemize}     
    ...
    Por ser uma aplicação em Ruby, onde os arquivos de código e de configuração são baseados em texto puro, o Ruby on Rails não exige nenhuma ferramenta avançada de criação.     Em outras palavras, utilizar um editor de texto simples ou uma IDE (Integrated Development Environment) é uma decisão pessoal.
    As IDE's mais famosas para trabalhar com Rails são:
\begin{itemize}
...
\end{itemize}     
    De acordo com a apostila da Caelum, a maioria dos desenvolvedores Ruby on Rails não utiliza nenhuma IDE, apenas um bom editor de texto e um terminal (ou prompt) de comando aberto.  Algumas ferramentas de texto boas para esse propósito, junto aos sistemas operacionais que suportam, são as seguintes:
\begin{itemize}
...
\end{itemize}
    ...
    \section{Criação de aplicação}
            Uma vez que o Rails tenha sido instalado corretamente e, automaticamente, definido como variável de ambiente no terminal, podemos rodar o comando ``rails'' para criação de aplicação e módulos.  Podemos verificar a versão do rails através do comando:
    rails -- version
           
    A versão utilizada nesse projeto é a 3.2.11, a última disponibilizada. A fim de criar a aplicação, utilizando o banco de dados Postgre, executamos o seguinte comando:
...
\begin{itemize}
\item App: contém os arquivos específicos da aplicação. Conforme citado no capítulo dois, o rails utiliza o padrão MVC, e dentro desse diretório a divisão é feita através dos sub-diretórios model, view e controller;
...
\item test: utilizado para testar a aplicação, normalmente quando se usa desenvolvimento orientado a testes (TDD). Em nosso projeto, não será utilizado.
...
\end{itemize}     
    ...
    \subsection{PostgreSQL}
            Banco de dados são coleções de informações que se relacionam de forma que crie um sentido. Seu objetivo principal é representar abstratamente uma parte do mundo real, conhecida como Universo de Discurso. (http://www.dct.ufms.br/~edson/bd1/bd1.pdf).
    Em nossa aplicação, utilizamos um banco de dados relacional chamado PostgreSQL. Lançado em 1995, está atualmente na versão 9.1.4, sobre a licença BSD.
    Nossa opção pela escolha do PostgreSQL deve-se ao bom suporte do Heroku a este SGBD. Entretanto, outros motivos poderiam ser colocados em prol dessa decisão, a saber:
\begin{itemize}
...
\end{itemize}
    \subsection{Modelos}
            Conforme o diagrama de classes citado no capítulo três, a aplicação possui algumas entidades. Neste momento, entendemos entidade como uma tabela presente no banco de dados e um active record contido na pasta app/models.
            O Rails possui uma forma bem simples de criar modelos, através de um comando de criação. Tomemos, por exemplo, a entidade Usuario, tal que seus campos e tipos de campos sejam passados como parâmetros de um comando de criação de modelos:
...
\end{itemize}     
    ...
    \subsection{Relacionamentos}
            ...
            Ao analisarmos diagrama de classes da aplicação na seção 3.x, verificamos que existem relacionamentos entre as entidades apresentadas. Tais entidades representam relações entre tabelas do banco de dados, as quais devem ser implementadas na camada de modelo da aplicação.
...
\begin{itemize}
\item $Belongs_to$: é utilizado quando um modelo tem como atributo o id de outro modelo (em um relacionamento um para muitos ou um para um).
\item $has_many$: associação contrária ao $belongs_to$; indica que um determinado modelo tem muitas instancias de outro modelo
\item $has_one$: similar ao $has_many$, porém com apenas uma instância (relacionamento um para um)
\item $has_and_belongs_to_many$: associação muitos para muitos
\end{itemize}     
...
\end{lstlisting}
    Nesses modelos, definimos que pergunta pertence a quis, e que um quiz possui várias perguntas. Tais definições criam métodos de acesso automaticamente, do tipo:
\begin{lstlisting}   
 quiz.perguntas #retorna array de perguntas
    pergunta.quiz #retorna objeto Quiz
\end{lstlisting}     
    O Rails não verifica automaticamente se a relação declarada funciona. Porém, de acordo com o princípio ``convenção sobre configuração'', definido na seção 2.x, é esperado que existam chaves estrangeiras do tipo $<modelo>_id$ nas tabelas que possuam relacionamentos do tipo $belongs_to$. Para tanto, devemos criá-las nas tabelas do banco, utilizando um objeto migration.
            Para criar um objeto migration, entre quiz e perguntas, utilizamos o seguinte generate do rails:
            Rails generate migration AddColumnQuizIdPergunta
     
    Isso gera um arquivo migartion chamado $``<data_hora> add_column_quiz_id_pergunta''$, no diretório /db/migrate. Implementamos, então, o migration com o campo id:
\begin{lstlisting}
 ...
 \end{lstlisting}    
    Ao rodar o script de execução, rakedb:migrate, será executado um SQL de criação da coluna no banco de dados. É recomendado que as alterações sofridas no banco de dados ao longo do projeto utilizem migration, uma vez que é possível registrar toda a seqüência de alterações no banco e reproduzi-las em outros ambientes.
     
     
    \section{Rotas e REST}
            As rotas no rails servem para transformar determinadas requisições URI em chamadas para controles particulares.  Algumas dessas rotas, para recursos, são utilizadas para o desenvolvimento da aplicação utilizando REST, em conformidade ao definido na seção 2.x.
            Aplicações em Rails são ditas Restful, associando o o protocolo HTTP às operações de CRUD. Em nossa aplicação, temos as seguintes operações disponíveis:
\begin{itemize}
\item Get: retorna o valor ou representação do recurso - select
\item Post: criação de um novo recurso - insert
\item Put: altera o recurso - update
\item Delete: remove o recurso - delete
\end{itemize}     
    Para acessar o recurso, definimos uma rota de recurso no arquivo routes.rb. Seja o modelo de quiz; definimos o seguinte recurso:
\begin{lstlisting}
    # routes.rb
    resources :quizzes
 \end{lstlisting}      
...
\end{itemize}     
    ...
    Os resultados dos métodos invocados pelas rotas, por padrão, são em HTML. Entretanto, existem facilitadores no Rails para que esse retorno seja em XML ou JSON. Além disso, podemos passar no corpo de um HTTP uma entrada em XML/JSON, de modo a ser interpretada pelo controller.
            Em nossa aplicação, toda a comunicação entre o Web Service (aplicação rails) e o cliente (aplicação mobile) é feita através de Rest utilizando JSON. Isso é possível utilizando um Format, como no exemplo abaixo:
\begin{lstlisting}
    # GET /quizzes
      # GET /quizzes.json
    def index
 ...
 \end{lstlisting} 
            O método index é responsável por retornar um array com todos os quizzes. Se a requisição vier via HTML, ou seja, do próprio site, a resposta será em HTML; caso contrario, a resposta será em JSON. Dessa forma, percebemos que a forma como os dados são requisitados é que diferenciam a saída .
    ...
    \section{Autenticação}
            ...
            Em nossa aplicação, a autenticação é feita através de tokens. Tokens são chaves criptografadas que identificam uma sessão do usuário. Optamos por disponibiliza-los no momento que o usuário realiza o login. Uma vez que o browser (ou o mobile) obtenha o token, o acesso é garantido até que esse token expire.
...
\end{itemize}     
    ...
    Figura 1Tela mostrada quando o usuario tenta acessar recurso e nao está logado
     
    Figura 2Edição de dados do usuário
     
    Figura 3Lembrar senha do usuário
...
\begin{lstlisting}
 ...
 \end{lstlisting} 
    ...
    \subsection{Bootstrap}
            ``Elegante, intuitiva e poderoso framework de front-end para desenvolvimento web rápido e fácil'' - com essa descrição, o Bootstrap é apresentado em seu site oficial \url{http://twitter.github.com/bootstrap/}.  Lançado em Agosto de 2011 pelo Twitter, é atualmente o projeto mais popular do GitHub. \url{https://github.com/popular/starred}
            O Bootstrap é uma coleção livre de ferramentas para desenvolver sites e aplicações web. Basicamente, contém templates HTML, JavaScript e CSS para tipografia, formas, botões, gráficos, navegações, etc.
    Em nossa aplicação, utilizamos o Bootstrap para personalização de botões, tabelas, divisão da página em duas seções e menu.
     
    Figura 5Exemplo de controle bootstrap
    ...
    Figura 6Visualização em desktop, tela larga
    ...
    rails server
            ...
            O nosso ambiente de produção está hospedado como serviço no Heroku, sobre o runtime stack ``Celadon Cedar''. Para ativar, foi necessário fazer o cadastro, instalar o Heroku Toolbelt, efetuar login via terminal e fazer deploy. Mais detalhes podem ser encontrados em \url{https://devcenter.heroku.com/articles/quickstart}.
